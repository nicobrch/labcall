	...
	const updateStudentAxis = (student: Student, axis: Axis) => {
		...
		newStudents.forEach((newStudent) => {
			if (newStudent.id === student.id) {
				...
				if (axisIndex !== -1) {
					console.log("Eje ya existe en estudiante, actualizando habilidades...");

					newStudent.axis[axisIndex].abilities = axis.abilities;
				} else {
					console.log("Eje no existe en estudiante, agregando o modificando eje vacío...");
	...
	return (
		<>
					...
					<table className="w-full table-auto py-3">
						<thead>
							<tr className="bg-gray-2 text-left dark:bg-meta-4">
								...
								<th className="font-medium text-black dark:text-white px-2 text-start py-3">Nombre</th>
								<th className="font-medium text-black dark:text-white px-2 text-start py-3">Eje/Habilidad</th>
							</tr>
						</thead>
						...
						<tbody>
							...
							{students.map((student, key) => (
								<tr key={key}>
									<td className={"border-t border-[#eee] pl-4 dark:border-strokedark xl:pl-11"}>
										<div className="relative z-20 flex items-start">
											<input
												type="checkbox"
												className="w-5 h-5 border border-stroke rounded-sm text-primary focus:ring-0"
												onChange={(e) => onChangeStudentCheckbox(student, e.target.checked)}
											/>
										</div>
									</td>
									<td className={"border-t border-[#eee] pl-4 dark:border-strokedark xl:pl-11"}>
									...
									<td className={"border-t border-[#eee] pl-4 dark:border-strokedark xl:pl-11 hidden sm:block"}>
										{student.axis.map((eje, index) => (
											<Selects key={index} student={student} selectedAxis={eje} updateStudentAxis={updateStudentAxis} removeStudentAxis={removeStudentAxis} />
										))}
									</td>
									<td className="block md:hidden">
										<div className="items-center justify-center">
											<button
												className="inline-flex items-center justify-center rounded-full bg-black py-3 w-35 text-center font-medium text-sm text-white hover:bg-opacity-90 lg:px-8 xl:px-10 mr-2"
												onClick={() => {
									...
									<td className={"pl-4 dark:border-strokedark p-3 xl:pl-11 hidden sm:block items-end justify-end"}>
										<button
											className="inline-flex items-center justify-center rounded-md bg-black p-4 text-center font-medium text-sm text-white hover:bg-opacity-90 h-4 w-4"
											onClick={() =>
			...
			<Modal
				...
				body={<MultiStudentSelects students={selectedStudents} axis={multiStudentAxis} setAxis={setMultiStudentAxis} />}
				show={showModal}
