  ...
  if (req.method === "POST") {
    try {
      ...
      form.parse(req, async (err, fields, files) => {
        ...
        const currentUsers = await getStudents(parseInt(courseId));
        // Procesar JSON
        ...
        if (studentsInsertedCount === 0) {
          res.status(400).json({
            error: `No se insertaron estudiantes. Verifica el contenido del archivo.`,
          });
          return;
        } else {
          res.status(200).json({
            message: `Se insertaron exitosamente ${studentsInsertedCount} estudiantes.`,
          });
          return;
        }
      });
