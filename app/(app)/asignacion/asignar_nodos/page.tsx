...
const CrearCuestionario = () => {
  ...
  const [courses, callCourses, statusCourses, errorCourses] =
    useCallGetApi("/course/students");
  const [abilities, callAbilities, statusAbilities, errorAbilities] =
    useCallGetApi("/abilities");
  ...
  const onChangeSelectedCourse = (courseValue: IOption) => {
    const course = courses.find(
      (course: ICourse) => course.name == courseValue.label
    );
  ...
  const onChangeSelectedUser = (userValue: IOption[]) => {
    const users = selectedCourse?.usuarios?.filter((user: IUser) =>
      userValue.some((u) => u.label == user?.firstname + " " + user?.lastname1)
    );
  ...
  const setSelectedAxisForNode =
    (axis: IOption, index: number) => (axis: IOption) => {
      const newNode = [...nodes];
  ...
  const setSelectedAbilitiesForNode =
    (abilities: IOption[], index: number) => (abilities: IOption[]) => {
      const newNode = [...nodes];
  ...
  const addEmptyNode = () => {
    ...
    if (
      nodes.some(
        (node) =>
          node.selectedAxis.label === "Selecciona un eje" &&
          node.selectedAxis.value === ""
      )
    ) {
      return;
    }
    ...
    setNodes([
      ...nodes,
      {
        selectedAxis: { label: "Selecciona un eje", value: "" },
        selectedAbilities: [],
      },
    ]);
  };
  ...
  const getAvailableAxisOptions = () => {
    ...
    return axisOptions?.filter(
      (axis: IOption) =>
        !nodes.some((node) => node.selectedAxis.value === axis.value)
    );
  };

  const buildNodes = (nodes: Node[]) => {
    const nodosOneAbility: NodeOneAbility[] = [];
    nodes.forEach((node) => {
      node.selectedAbilities.forEach((ability) => {
        nodosOneAbility.push({
          axis: node.selectedAxis.label,
          ability: ability.label,
        });
      });
    });
    return nodosOneAbility;
  };

  const saveNodes = async () => {
    let _nodes = buildNodes(nodes);
  ...
  return (
    <div>
      <div className="absolute z-9999 top-5 right-5 ">
        <AlertConfirmacion
          title={"¡Nodos asignados con éxito!"}
          body={"Ya puedes ver las preguntas."}
          show={showAlertOK}
          setShow={setShowAlertOK}
        ></AlertConfirmacion>
        <AlertError
          title={"No se pudo agregar nodos :("}
